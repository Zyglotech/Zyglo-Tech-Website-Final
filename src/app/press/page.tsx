  ...
  return (
    <main className="min-h-screen" style={{ background: '#060B17' }}>
      <section className="mx-auto max-w-7xl px-5 py-16 lg:px-8 lg:py-24">
        ...
        <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-cyan-400/20 px-4 py-1.5" style={{ background: 'rgba(6,204,232,0.08)' }}>
          <Megaphone className="h-3.5 w-3.5 text-cyan-400" />
          <span className="text-[11px] font-semibold uppercase tracking-[0.18em] text-cyan-400">Newsroom</span>
        </div>
            ...
            <div>
              ...
              <div className="space-y-6">
                {pressMentions.map((mention, idx) => (
                  <div key={idx} className="group rounded-2xl border border-white/[0.07] p-6 transition hover:border-cyan-400/20" style={{ background: '#0B1424' }}>
                    ...
                    <h3 className="text-[18px] font-bold leading-snug text-white group-hover:text-cyan-400 transition">
                      {mention.title}
                    </h3>
                    <p className="mt-3 text-[13.5px] leading-6 text-slate-400">{mention.excerpt}</p>
                    <a href={mention.link} className="mt-4 inline-flex items-center gap-1.5 text-[12px] font-semibold text-cyan-400 hover:text-cyan-300">
                ...
                {pressReleases.map((pr, idx) => (
                  <a key={idx} href={pr.link} className="group flex items-center justify-between p-5 transition hover:bg-white/[0.02]">
                    <div>
                      ...
                      <p className="text-[14px] font-bold text-white group-hover:text-cyan-400 transition">{pr.title}</p>
                    </div>
                    <ArrowUpRight className="h-4 w-4 text-slate-600 group-hover:text-cyan-400 transition shrink-0" />
                  </a>
                ))}
              </div>
            </div>
          ...
          <div className="space-y-8">
            ...
            <div className="rounded-2xl border border-white/[0.07] p-6" style={{ background: '#0B1424' }}>
              ...
              <div className="space-y-3">
                <a href="mailto:founder@zyglotech.com?subject=Media%20Kit%20Request" className="flex items-center justify-between rounded-xl bg-white/5 p-3 text-[13px] font-medium text-white transition hover:bg-white/10">
                  <span className="flex items-center gap-2"><BrandLogo className="h-4 w-4" /> Request Brand Assets</span>
